...
void Return::gencode(bool addr,bool is_struct){
	...
	for(int i=0; i<width;i++){
		addcode("sw "+rstack.top()+", "+to_string(i*4+w)+"($fp)");
		...
		if(left->label <REG_NUM || right->label<REG_NUM){
			...
			if(left->label>right->label){		//left label greater
				...
				if(op->op == "DOT_OP" || op->op == "PTR_OP"){	//struct related statements
					...
					int rwidth;
					if(right->exp_type[0].substr(0,5)=="array"){
							right->gencode(true,false);
							rwidth=1;
					}
					else{
						right->gencode(false,false);
					 	rwidth = getWidth(right->exp_type)/4;
					}
					rfloat = cast(right);
					vector<string> temp;
...
void func::gencode(bool addr, bool is_struct){
	...
	addcode("addi $sp, $sp, -4        #space for SL");    //space for SL & RA
	addcode("jal "+fun_name+"           #call to "+fun_name);
	vector<string> typ=getGlobalType(fun_name,glob);
	if(typ[0]=="float"){															//struct to be handled
		addcode("lwc1 "+fstack.top()+",0($sp)");
		addcode("addi $sp, $sp, 4");
	}
	else{loadFromStack(rstack.top());}
...
void array_ref::gencode(bool addr, bool is_struct){
	if(child->label <REG_NUM || vec->label<REG_NUM){
		if(child->label>vec->label){
			if(child->exp_type[0]=="pointer")
				child->gencode(false,is_struct); 			 //deref lhs
			else 
...
void addLocals(){
	...
	addcode("addi $sp, $sp, "+to_string(-w));
}
...
bool cast(exp_ast* node){
	if(node->typecast=="TO-INT"){
		...
		addcode("cvt.s.w "+fstack.top()+", "+fstack.top());
		addcode("addi $sp, $sp ,4");
		return true;
	}
...
void function_return(){
	...
	int w=getParamWidth(loc)+12;
	addcode("addi $sp, $sp, "+to_string(w)+"                  #stack restore");
	addcode("jr $ra");
}
